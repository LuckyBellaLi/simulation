...
By default, you don't need to change anything, and directly run the code to get results for DE.

Since there are random numbers generated in the code, each run of the code may get a slightly different result.

Please look at paper seciton 5.3. Double Estimator for the DE method.
...
for r in range(0,replication):   
    ...
    mu_index=candidate_result.index(min(candidate_result))#get mean for generating Z
  
    cclb1=0
    cclb2=0
    
    for i in range(0,batch):
        for j in range(0,m1):#this for loop use IS to generate quantile and mean estimators
            #generate Z with mean mu
            Z=np.random.normal(loc = mu, scale = 1, size =d)
